...
export class AuthTeacherController {
  ...
  @Post('logout')
  @ApiOperation({ summary: 'Logout teacher user' })
  @ApiCreatedResponse({
  ...
  async forgotPassword(@Body() { email }: ForgotPasswordDto): Promise<void> {
    const user = await this.authService.validateTeacherEmail(email);
    ...
    const link = `https://localhost:5432/auth/teacher/${user.id}/password/recover?token=${oneTimeRecoveryToken}`;
  ...
  async recoverPassword(
    ...
    const user = await this.authService.validateTeacherId(teacherId);
